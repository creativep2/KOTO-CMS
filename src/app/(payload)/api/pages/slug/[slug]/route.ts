  ...
  try {
    ...
    if (!validLocales.includes(locale)) {
      const response = NextResponse.json(
        { error: 'Invalid locale. Supported locales: en, vi' },
        { status: 400 },
      )
      return addCorsHeaders(response)
    }
    ...
    if (result.docs.length === 0) {
      const response = NextResponse.json({ error: 'Page not found' }, { status: 404 })
      return addCorsHeaders(response)
    }

    // Return the page directly (not wrapped in docs array)
