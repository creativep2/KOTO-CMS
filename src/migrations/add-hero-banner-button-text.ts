...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  console.log('Adding button_text field to hero_banners_locales table...')
  ...
  try {
    ...
    if (!tableExists.rows[0].exists) {
      console.log('hero_banners_locales table does not exist. Creating it first...')
    ...
    if (!columnExists.rows[0].exists) {
      // Add button_text column to existing hero_banners_locales table
      await db.execute(sql`
        ALTER TABLE hero_banners_locales 
        ADD COLUMN button_text TEXT
      `)
      console.log('Added button_text column to hero_banners_locales table')
    } else {
      console.log('button_text column already exists, skipping...')
    }
    
    console.log('Successfully added button_text field to hero_banners_locales table')
  } catch (error: any) {
    console.error('Error adding button_text field:', error)
    throw error
  }
}
